...
import com.example.guessnumberservice.exception_handler.exception.DuplicateException;
...
public class AuthServiceImpl implements AuthService {
    ...
    public void register(RegisterRequest request) {
        if (userRepository.findByUsername(request.getUsername()).isPresent()) {
            throw new DuplicateException(messageUtil.getMessage("username.exists"));
        }

        User user = new User();
        user.setUsername(request.getUsername());
        user.setPassword(passwordEncoder.encode(request.getPassword()));
        user.setScore(ConfigGame.DEFAULT_SCORE);
        user.setTurns(ConfigGame.DEFAULT_TURN);
    ...
    public AuthResponse login(LoginRequest request) {
        User user = userRepository.findByUsername(request.getUsername())
                .orElseThrow(() -> new BadCredentialsException(messageUtil.getMessage(ConstMessageError.Auth.LOGIN_INVALID)));

        if (!passwordEncoder.matches(request.getPassword(), user.getPassword())) {
            throw new BadCredentialsException(messageUtil.getMessage(ConstMessageError.Auth.LOGIN_INVALID));
        }
